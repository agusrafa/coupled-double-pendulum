...
var g=9.81; //(m/s2)
var h=1/60.0; //incremento h - puede ser 1/60, lo achique para que se vea mas rapido
...
var x1=eq1+A0,x2=eq2+A1,v1=0.0,v2=0.0,r=0.025,y=0.5; //condiciones de borde
var mass_1 ={
            m:m,//en kg
            radio: r, //en m
            pos : {x: x1, y: y}, //en m
...
var inicio=false;
var tiempo=0,bandera=false;


function GetValue(){
  bandera=true;
  const valor1=document.getElementById("k");
  const valor_1=Number(valor1.value);
  if(valor_1<=0&&valor_1>100){
    Swal.fire({
      title:'Error',
      text:'El valor de la constante de elsaticidad k debe ser mayor a 0 y menor a 100.',
      icon:'error'
    });
  }else{
    k=valor_1;
  }
  const valor2=document.getElementById("A1");
  const valor_2=Number(valor2.value);
  if(valor_2<-0.04||valor_2>0.04){
    Swal.fire({
      title:'Error',
      text:'El valor de amplitud A1 debe estar entre los valores -0,04 y 0,04, sino no se cumple la aproximación de angulos pequeños.',
      icon:'error'
    });
  }else{
    A0=valor_2;
  }
  const valor3=document.getElementById("A2");
  const valor_3=Number(valor3.value);
  if(valor_3<-0.04||valor_3>0.04){
    Swal.fire({
      title:'Error',
      text:'El valor de amplitud A2 debe estar entre los valores -0,04 y 0,04, sino no se cumple la aproximación de angulos pequeños.',
      icon:'error'
    });
  }else{
    A1=valor_3;
  }
  const valor4=document.getElementById("m");
  const valor_4=Number(valor4.value);
  if(valor_4<0){
    Swal.fire({
      title:'Error',
      text:'El valor de la masa m debe ser mayor a 0.',
      icon:'error'
    });
  }else{
    m=valor_4;
...
function InicioPausa(){
  if(bandera==true){
    if(inicio==true){//para poner pausa
      inicio=false;
      tiempo=t0;
    }else{//para poner play
      inicio=true;
      t0=tiempo;
    }
  }   
